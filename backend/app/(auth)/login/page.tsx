...
// Calls POST http://localhost:5000/auth/login → gets JWT token → stores it
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Store token in localStorage AND cookie (cookie used by middleware)
      setToken(token)
      setStoredUser(user)
      document.cookie = `tcb_token=${token}; path=/; max-age=${60*60*24*7}` // 7 days
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'grid', gridTemplateColumns:'1fr 1fr' }}>
      ...
      <div style={{ background:'var(--bg-1)', borderRight:'1px solid var(--border)',
        ...
        <Link href="/blog" style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:16, color:'var(--text-1)' }}>The Corporate Blog</span>
        </Link>

        <div>
          <blockquote style={{ fontFamily:'var(--font-serif)', fontSize:24, lineHeight:1.4,
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:'50%', background:'var(--gold-dim)',
              display:'flex', alignItems:'center', justifyContent:'center', color:'var(--gold)', fontSize:16, fontWeight:600 }}>
              S
            </div>
            <div>
      ...
      {/* Right — Form */}
      <div style={{ display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'48px 56px' }}>
        <div style={{ width:'100%', maxWidth:380 }}>
          <h1 style={{ fontFamily:'var(--font-serif)', fontSize:32, fontWeight:500,
            marginBottom:6, letterSpacing:'-.02em', color:'var(--text-1)' }}>Welcome back</h1>
          <p style={{ fontSize:14, color:'var(--text-3)', marginBottom:36 }}>Sign in to your editorial dashboard</p>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div>
              ...
              <div style={{ textAlign:'right', marginTop:8 }}>
                <Link href="#" style={{ fontSize:11, color:'var(--gold)' }}>Forgot password?</Link>
              </div>
            </div>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              color:'#0A0A0B', fontSize:13, fontWeight:600, cursor:'pointer', border:'none',
          ...
          <p style={{ fontSize:12, color:'var(--text-3)', textAlign:'center' }}>
            Don't have access?{' '}
            <Link href="/register" style={{ color:'var(--gold)', fontWeight:500 }}>Request access →</Link>
          </p>
        </div>
      </div>
    </div>
